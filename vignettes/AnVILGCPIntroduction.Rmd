...
```{r gcs-list-stat, eval=has_gcloud}
avlist(src)
...
```{r gsutil-rsync, eval=has_gcloud}
...
```{r workspace-data-image, echo = FALSE, cache = FALSE}
...
```{r workspace-setup-hidden, include=FALSE, cache=FALSE, eval=has_gcloud}
...
```{r workspace-info, eval=has_gcloud}
...
```{r avtables-list, eval=has_gcloud}
avtables()
...
```{r avtable-manipulate, eval=has_gcloud}
sample |>
...
```{r avtable-import-status, eval = FALSE}
...
```{r avtable-import-demo, eval = FALSE}
...
```{r avtable-import-set, eval = FALSE}
